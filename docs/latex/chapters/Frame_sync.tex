...
\todo[inline]{Este algoritmo se tiene que cambiar por completo. Hay que cambiar por completo este capítulo. Quiero seguir la estructura y tal.
hay que cambiar la gráfica del BER para poner la curva nueva en vez de la azul. De este capítulo, el phase ambiguity
se cambia: mira el algoritmo implementado en el código y detalla en el informe como se hace. Tampoco escribass demasiado, ajústate y hazte una idea de lo qeu quiero con lo que ya hay
. Hay que cambiar la figura 4.4. Probblemente haya que cambiar el punto 4.2 y 4.3}
