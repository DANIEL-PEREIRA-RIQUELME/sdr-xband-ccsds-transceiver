...
\todo[inline]{Esto hay que cambiarlo. Todo lo que pasa a continuación en este capítulo hay que cambiarlo desde aquí hasta el final de este. Contar que al principio se pensó usar Gpredict praa estimar y corregir el doppler seguido de una estimación fina usando costas loop (manén las estructura del capítulo) pero al final se eligió hacer el aloritmo de compensación en c++ que detallaste elevando a la 4 potencia ademaś de una FFT para ver el pico y aplicar un plinomio. Hablar del consumo de este bloque y el resultado que da, el doppler residual (tanto Hz de doppler como Hz/s de rate) que deja para el costas loop
. Comentar que en un principio se eligió gpredict pero no es suficiente: los errores pueden ser muy grandes y ya fueron estudiados (poner la subsección de 
hardware drifts and residual error analysis como apéndice dedicado). Hay más errores como hardware drifts y errores de TLE cuando hay por ejemplo tormentas solares o cuando no hay TLE actualizados
recientemente. Además, cuando el satélite se lanza los TLE tardan en estabilizarse. Además, depender de un software externo puede ser desventaja (hablar de todas las implicaciones e inconvenientes).
Todo ello provoca que el costas loop trabaje bajo presión cosa que no debería ser el caso pra una estimación fina. El costas loop debía de configurarse con un
ancho de banda grande para poder lidiar con estos problemas probocando caídas en la SNR. Así pues, se justifica el uso de este algoritmo 4th potencia y FFT+.... para lidiar
todos estos errores. Comenta el consumo y eficiencia. Además, da mejores resultados en BER. El tema de Fine frequency tuning:the costas lloop no tienes que 
cambiarlo demasiado. Solo quiero que se vea que lo que necesitamos de BW en base a lo que devuelve el algoritmo de 4th potencia y FFT+... es mucho menor que lo que se necesitaba con Gpredict. Comenta que el costas loop es un algoritmo muy eficiente y que no consume muchos recursos. 
Por tanto cualquier comentario relacionado con que necesito tal BW para tal Hz/s residual de gprdict elimínalo. }
